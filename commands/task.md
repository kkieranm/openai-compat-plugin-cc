...
- Preserve the user's own flags (`--provider`, `--model`, `--base-url`, `--timeout`, `--max-seconds`, `--max-attempts`, `--max-tokens`, `--temperature`, `--reasoning-effort`, `--top-p`, `--top-k`, `--min-p`, `--presence-penalty`, `--system`, `--template`) exactly as given.
- `--reasoning-effort <level>`, `--top-p <n>`, `--top-k <n>`, `--min-p <n>` and `--presence-penalty <n>` set the model's own sampling and reasoning knobs, each sent under its OpenAI field name. `--reasoning-effort` takes one token (e.g. `low`, `medium`, `high`, `xhigh`) passed through for the server to accept or refuse; the rest are numbers — `--top-p`/`--min-p` in 0–1, `--top-k` a positive integer, `--presence-penalty` in −2 to 2. Use them to run a model at the settings its vendor recommends: some models ship with a verbose reasoning effort that spends the whole budget thinking and never writes an answer, and `--reasoning-effort low` is the fix.
