...
The script decides what to review — do not build a diff yourself or paste one into the arguments. With no flags it reviews uncommitted work (working tree, staged changes, and untracked files). `--staged`, `--base <ref>`, `--commit <ref>` and repeated `--file <path>` change the target; `--provider`, `--model`, `--base-url`, `--timeout`, `--max-seconds`, `--max-attempts`, `--max-tokens`, `--temperature`, `--reasoning-effort`, `--top-p`, `--top-k`, `--min-p` and `--presence-penalty` behave as in `/oai:task` — the sampling and reasoning knobs are sent under their own OpenAI field names, `--reasoning-effort low` being the one that rescues a model whose default effort never finishes an answer — except that `--max-tokens` has a floor here: a review reply carries a findings array and a reasoning field, and a budget too small to hold both is refused rather than sent, naming the minimum; and except that `--max-seconds` bounds only the original review request here — a review cut off mid-answer may run a salvage follow-up on its own separate budget, so the wall clock can exceed `--max-seconds` by up to 600s (`/oai:task`, which has no salvage, is bounded by it). Pass the user's flags through exactly. Any trailing text is forwarded as extra instructions for the reviewer, verbatim.

Each changed file is sent whole alongside the diff, so the model can resolve anything defined outside the changed hunks — provided the provider's context window can be determined. Where it cannot, the diff-covered changed files are not sent whole — the rung is skipped rather than attempted unmeasured — and the report says so and names the remedy. Files covered by no diff (untracked, or given with `--file`) still go whole in that case, because withholding the only copy of that code would review nothing; those bodies go unmeasured too. `--diff-only` sends just the diff — faster on a slow local model, at the cost of the "X is not defined" false positives that whole files exist to prevent. It cannot be combined with `--file`, which has no diff.
...
`--passes N` runs the review N independent times and reports the **deduplicated union** of their findings, with a per-finding count of how many passes reported it — a single pass is a lottery (identical input has produced one real defect on one run and nothing on the next), and agreement across independent passes is the closest thing to a free verifier. It is off by default (`--passes 1` is the ordinary single pass, byte-for-byte). Findings are merged by **location** (same file and line), and the count is how many passes flagged that location — not proof they found the same defect. It is a **lower bound** on true agreement (a paraphrased or off-by-a-line duplicate does not merge), and where the passes' summaries differ it may join **different** defects reported at one line; the report shows all the differing summaries so the count is never read as agreement on one defect. A pass that returned an empty findings list still counts as a pass that looked and found nothing; a pass that failed or whose reply could not be read is named but excluded from the count. The passes run one after another (one local model), and `--max-seconds` bounds **each** pass, so the wall clock scales with N. `--temperature 0` makes the passes near-identical, which makes their agreement say little — leave the default temperature to get the decorrelation the flag exists for. The run is refused if the server does not confirm which model answered each pass, or if two passes were served different models, since a union across models is not a measurement of one.

`--lens a,b,c` runs one pass per named **lens** — a focus the pass is told to prioritise — and reports the same deduplicated union, with each finding tagged by the lens(es) that flagged it. The lenses are a fixed set: `correctness`, `security`, `edge-cases`. Where `--passes` decorrelates sampling noise (the same prompt run again), lenses decorrelate **blind spots**: a finding a single lens flagged is a blind spot that lens was looking for, so a lens run reports **coverage across focuses, not confidence** — being flagged by fewer lenses is not a demotion, and the report says so. It is mutually exclusive with `--passes` (the two are different pass strategies and are refused together, not silently merged) and runs the passes one after another like `--passes`, so the wall clock scales with the number of lenses. The lens is added at the **tail** of the request, after the diff, so the passes of one lens run still share a cached prefix; the same model must be confirmed across passes, exactly as for `--passes`.

`--json` prints the whole run as one JSON object on stdout — findings, summary, every caveat the text report carries, plus token usage and timing — instead of the report below. It exists for scripts and for the benchmark harness (`npm run bench`). Pass it only if the user asks for it: the verification duty below still applies to whatever it returns, and `"parsed": false` means the model's reply could not be read, which is not the same as finding nothing. Under `--passes N` or `--lens a,b,c` the object instead carries `kind: "multi-pass-review"`, a `passes[]` array of per-pass records (each with its own `lens`, or `null` on the `--passes` path), and a top-level `strategy` (`"passes"` or `"lenses"`) and `lenses` list. On the `--passes` path each finding carries `agreement` (how many passes reported it) and `readablePasses` (how many passes were counted). On the `--lens` path each finding carries `lenses` (the lens(es) that flagged it) *instead* — there is no `agreement`, because with one pass per distinct lens it would just equal the number of lenses and invite a confidence reading of what is coverage across focuses.
