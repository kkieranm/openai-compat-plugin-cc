...
is **not** an outage — and, per `bench/lib/sweep-health.mjs` `replayStreak`, resets any live outage
...
- `.claude/REPO_TRAPS.md` — one entry (review-ladder batch 1, under the ladder's own confirmed-class rule):
  a rendered claim about a state transition pinned only by a surrogate predicate, naming the
  `replayStreak` test that guards it.
...
`starved` paragraph (the first build added a `failed` sentence there; it duplicated `failedExplanation`'s
own docstring and was dropped); `starvedExplanation`'s docstring, whose `hasOwn`/value-shape paragraph now
describes `ownProse`, is reduced to its own two-not-usable-arms/table/fallback behaviour.
...
response**, before any content or reasoning text arrived — so it is neither a transport failure nor a
dropped request, and it is **not retried**: this client classifies such a refusal as non-retryable. The
row counts as a failed attempt and says nothing about server health.
```
...
1. **`a failed stream-error-frame commit is explained as an in-stream refusal, not a generic failure`**
   ...
   still appended), matches `/resets any live outage streak/` (and pins the unconditional `/resets a live outage streak/` OUT), and does NOT match `/the review failed/`.
...
3. **`a FAILED_WHY entry that claims the row resets the outage streak is one isOutage rejects`** — for
   ...
   above was checked against nothing')`, so a rewording that drops the phrase, or an emptied table,
...
6. **`stream-error-frame is explained as a refusal inside a successful response's stream, never a transport
   failure or a drop`** — `paragraphAbout(renderWith('stream-error-frame'), 'stream-error-frame')`; assert
   `/refused inside the stream of a successful HTTP response/`, `/before any content or reasoning text/`, `/\*\*not retried\*\*/`,
...
7. **`an attempted stream-error-frame row resets any live outage streak, as its prose says`** and
   **`a stream-error-frame row with no live streak resets nothing`** (review-ladder batches 1 and 2) —
   drive `replayStreak` directly: [outage, refusal with `startedAt`, outage] → `resets 1`, `longest 1`,
   two outages; and the refusal first, or after a clean row with no prior outage → `resets 0`,
   `longest 0` — the `streak > 0` gate the conditional wording rests on.
...
- Mutation check on the key invariant, at the real path: delete the `'stream-error-frame'` key from
  ...
  `explanationFor`'s dispatch so `failed` rows skip `failedExplanation` → test 1 red. Third: add the
  words "outage streak" to the `REASON_PARAGRAPHS` entry → test 6 red (the leak guard fires). Fourth
  (batch witness): `replayStreak`'s reset predicate `if (streak > 0) resets += 1` mutated → test 7's
  streak case reds on `> 1`, its zero-streak case reds on an unconditional increment.
