...
## Errata — two claims below were adjudicated FALSE during review

**The body is left as approved and is not rewritten**; the approving round's archive sits beside it and
this section is what stops the errors standing unqualified. Both were found by the review ladder, and
both are corrected in the code and in `adr/014-async-jobs.md`. The reasoning is in
`plans/oai-66-the-worker-confirms-its-own-cancellation.ledger.md`.

1. *"Every failure of open, stat, read or close degrades to absent"* (Phase 1) — **a failed CLOSE does
   not.** By then the bytes have been read and compared, so a match stays a match. Only open, stat and
   read degrade. See ledger L35 / verdict-point V1.
2. *"anything able to write that directory can already write `jobs.db`"* (Phase 1) — **not in general.**
   The footing needs the whole permission chain: it fails exactly where an attacker can traverse the
   state directory, write `logs/`, and not write `jobs.db`. A state directory at `0755` over a
   plugin-created `logs/` at `0700` is safe, and `0777` lets that attacker replace `jobs.db` too, which
   collapses the distinction rather than widening it. See ledger L21 / V3, and **OAI-150**.
