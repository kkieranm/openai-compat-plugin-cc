...
- **Comparability (compare-model) refinements:** the lens-set axis is NOT sorted (execution order is
  ...
  input difference); and a present-but-non-string persisted `options.lens` reads fail-closed
  `unknown` on ALL THREE derived axes (`passes`/`strategy`/`lenses`) together, never as the plain
