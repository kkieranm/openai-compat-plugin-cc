...
- **Accepted cost, stated rather than mitigated**: an unknown window is *not* evidence of a small
  ...
  `/models` responses, Ollama's listing, and **any failed probe**. The remedy is the existing
  per-provider `contextLength`.
  **The per-vendor list is INFERRED FROM THE READERS IN `model-info.mjs`, NOT MEASURED**, and the
  correction belongs here as well as in the ADR (`adr/066`: fix the claim, not the instance). Only
  **LM Studio with a model resident** is observed — this session, `loaded_context_length` 61,696.
  llama.cpp, vLLM, TGI and oMLX have readers in `model-info.mjs` and are inferred from those. **Drop
  OpenRouter entirely**: there is no OpenRouter reader, `readVllm` keys on `max_model_len`, and
  nothing in this repo establishes what its `/v1/models` returns. A claim about what a value *reaches*
  must cite the endpoint.
...
**Phase 2 — CARRY the ladder's OBSERVED decision; do not re-derive its premise.** **Reversed
2026-08-12, mid-build episode 2**, at the user's decision, on `codex-adversarial`'s finding
(medium, confidence 0.98) and confirmed by this feature's own mutation 1.

The reversed text said: *"DERIVE the fact where it is rendered; do not thread a field … a stored flag
duplicates a fact already present at report time, and the derivation `!budget.checked &&
target.changed.length > 0` has every required negative behaviour on its own."* Every clause of that is
still true, **and it does not establish what it was used to establish.** The derivation restates the
ladder's *premise* — `budget.checked` is false exactly when `contextLength` is falsy, which is the same
truth test the ladder gets as `windowKnown` — rather than observing the *branch the ladder took*. The
two agree by **construction, not by observation**, so:

- Mutation 1 (removing the `windowKnown &&` guard) left the report asserting `skippedUnsizedWindow:
  true` while whole bodies went on the wire. The report lied and only the request-log test caught it.
- The plan's own justification — that a derivation *"cannot drift"* — is **false by construction**. It
  cannot drift from `budget.checked`. That is not the fact the field claims to report. The same false
  sentence reached `scripts/lib/review.mjs:114` and `adr/005:182` and is withdrawn with this phase.
- The field is consumed by `bench/lib/sweep-outcome.mjs` as **authoritative coverage evidence**, which
  is what makes a report that can contradict the request a correctness problem rather than a cosmetic
  one.

So `prepareLadder` returns what it **did**, and every rendering reads that:

- The whole-file rung returns `rung: 'whole'`; the fallback returns `rung: 'hunks'` plus

  ```js
  skipped: !windowKnown && target.changed.length > 0 ? 'unsized-window' : null
  ```

  `hunksOnly` keeps its exact current meaning and value on both rungs — this adds a cause beside it,
  and changes nothing about the state it reports.
  **The `changed.length > 0` half is not redundant and the gate caught its omission** (round 6): an
  earlier draft of this phase wrote `windowKnown ? null : 'unsized-window'`, which marks an
  unknown-window `--diff-only`, deletion-only or `--file`-only run as an unsized skip **when there
  were no diff-covered bodies to withhold**. That breaks the field's published `true iff` contract and
  would redden this feature's own negative controls. Both halves of the old derivation survive the
  reversal — what changes is that they are now evaluated **at the branch that acts on them**, once,
  rather than restated at the renderer.
- **`skipped` is `null`, never `false`, when the rung was taken deliberately.** A known-window oversize
  fallback was sized and shed; `null` says "no skip to explain", where `false` would assert a
  determination about a cause nobody evaluated.
- `review-request.mjs` and `cmd-review.mjs` thread that ladder result to the report, and
  `skippedUnsizedWindow` in the envelope becomes `ladder.skipped === 'unsized-window'`.
- **`skippedUnsizedWindow` keeps its name, its type and its published contract** — only its
  *provenance* changes. Nothing downstream of the envelope is touched, so phases 6 and 7 stand as
  built.
- **The mutation check is re-run and its meaning changes**: removing the ladder guard must now redden
  the request-log test **and** flip the disclosure, because both read one observation. That the two
  can no longer disagree is the whole point of the reversal, and the check must demonstrate it rather
  than assert it.
...
- `budget` is already destructured in `jsonReport` (`review-report.mjs:184`). `reportFindings`
  ...
  holds `budget` (`cmd-review.mjs:145`). **Still required** — `budget.checked` remains the
  footer's `contextNote` source; it is no longer the disclosure's source.
...
## Phase 8 — the three text findings from ladder pass 1 (added 2026-08-12, episode 2)

All three are accepted, none dismissed. Two are **ship-blocking adjudicated falsehoods** (`adr/083`):
a normative document may not state what a stage has adjudicated false.

**8a — `adr/005:81-84` states the opposite of shipped behaviour.** Adjudicated FALSE by
`codex-adversarial` (high, confidence 1.0), raised independently by the baseline-obligation audit as
O33. The paragraph says the incompleteness note *"is raised only when bodies we actually had were
dropped: under `--diff-only` none were collected"*. Shipped: `git-diff.mjs:219` empties `changed`
under that flag while still collecting the diff, `prepareLadder` returns `hunksOnly: hasDiff`, and
`tests/review-context.test.js:83` asserts the note fires. **Pre-existing — not introduced by this
feature**, and this is the second instance of the one claim (the first, `sweep-report.mjs`'s "did not
fit the window", was already fixed here), so it is discharged by a **repo-wide grep for the claim**,
never by an edit at the cited line (`adr/066`).

Rewrite the paragraph to separate the two kinds of note, which is the distinction the codebase now
actually holds: the **state** notice (`hunksOnly`) fires whenever a diff was reviewed without whole
diff-covered files — `--diff-only` included — and the **cause** notice (`skippedUnsizedWindow`) fires
only when collected bodies were withheld because the window could not be sized.

**8b — `adr/005:26`'s headline decision survives unconditional.** *"Each changed file is sent whole,
alongside the diff."* Phase 5's obligation is that the claim be conditional **wherever stated**, and
the amendment at the foot does not retract the headline. **Fix by FORWARD POINTER, not by
overwriting**: line 26 records what was decided on 2026-07-27, and rewriting it falsifies the history
the amendment exists to preserve. Same shape as the pointer already at `:77`, without the
strikethrough — it is the headline, and it was true when written.

**8c — an unevidenced vendor classification, authored by this feature.** The amendment's "Unaffected"
list names OpenRouter. Nothing in this repo establishes it: `model-info.mjs` has no OpenRouter
reader, `readVllm` keys on `max_model_len`, and a case-insensitive grep of the tree finds
"openrouter" only in this amendment and this plan. A claim about what a value **reaches** must cite
the endpoint. Either drop the unobserved entries, or mark the list as **inferred, not measured**, and
say which entries were actually observed and where.

## Still outstanding from pass 1, and NOT dispositioned by this amendment

`codex-plain`, `security-review` and `advisor-closer` never ran; `lean-wide` ran INCOMPLETE (its
digest guard fired on a one-character truncation — filed to the routing log as a `review-lean`
toolchain defect, not fixed here). All re-run when the pass resumes; none may be read as clean.

**`scripts/lib/review-unparsed.mjs` and `bench/lib/sweep-notes.mjs` remain UNREVIEWED executable
surface** (`adr/056`) — extracted under the size ratchet, read by no stage as new code. The resumed
pass is a **first review** of them, never evidence of convergence.
