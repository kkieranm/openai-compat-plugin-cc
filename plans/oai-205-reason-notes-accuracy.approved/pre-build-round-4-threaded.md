...
- Mutation check (the key invariant — each paragraph is gated on its own code): change one new
  gate's code string (`sawReason('empty-answer')` → `sawReason('empty-answr')`) via
