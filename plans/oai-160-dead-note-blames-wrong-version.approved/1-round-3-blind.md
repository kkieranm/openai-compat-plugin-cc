...
- Row schema known AND NOT `readOnly` → case (B): reconciliation DID run this pass and found
  nothing wrong at the time; the row's worker state changed in the narrow window between that check
  and this render. Not a version story at all.
...
helpers have no seam to control. It is documented as untested by design in Verification below,
per the `/feature` skill's step 5 exemption for a case that is "genuinely uncovered and costlier to
cover than the feature."
...
- Its `dead`/`never-started` branch (currently lines 143-145) becomes three-way, keyed on
  ...
  - Row schema known AND `readOnly` — new message naming the real cause: the database itself, not
    ...
    this run.`
  - Row schema known AND NOT `readOnly` — new message naming the race: something like `its own
    schema (${view.schema_version}) is understood and this build did reconcile the database this
    run, but the row still shows ${view.display} — its worker likely changed state in the moment
    between reconciliation and this render. Run /oai:status again to pick it up.`
...
- Case (B), the reconcile/render race, is stated as untested by design: it needs a process to die in
  a sub-millisecond window between two liveness probes inside one synchronous CLI invocation, which
  this repo's black-box test harness has no seam to force deterministically. The branch's logic
  (schema known, not readOnly → race wording) is otherwise the direct negation of the other two,
  already-tested/asserted branches, so its coverage gap is narrow and stated rather than silent.
