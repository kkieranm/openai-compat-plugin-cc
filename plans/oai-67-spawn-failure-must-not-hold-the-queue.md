...
the ledger at `scratchpad/ledger-oai-67.md`, and deliberately NOT here.** **That path is the SESSION
scratchpad, not a path in this repository — it does not resolve here and is not durable.** A reader
after the fact cannot follow it, so every measurement this plan relies on is stated where it is
relied on rather than cited to it; the ledger is working state, and its absence from the tree is the
expected condition rather than a missing file. A running total written into
...
and the evidence for it. The session ledger keeps the round-by-round history, subject to the
non-durability noted above.
...
- **`adr/014-async-jobs.md` — IN, for BOTH defaulted seams** (the injectable `spawn`, and the
  ...
  **Two PRE-EXISTING falsehoods in `adr/014` are also corrected, authorised by the user on
  2026-08-12** — recorded here on the same footing as `adr/020`'s three above, because the inventory is
  what makes an authorisation durable and a decision that lives only in a session is one nobody can
  check afterwards. Both were found by `codex-adversarial` in review pass 2, verified against the code,
  and put to the user as a `widening` under `adr/056` before any edit: (1) the eligibility-transaction
  sentence said "the registration" where only the OWNER registration is inside the transaction — the
  waiter registration is the statement printed above it, annotated "long before eligibility" — so it is
  qualified by one word, restoring a split this repo already made deliberately; (2) the `attachments`
  shape claimed `{path, bytes, sha256}` while `digestsOf` has only ever persisted `{path, bytes}`, so
  the hash is removed rather than the code changed. Codex steered FIX and the author recommended FIX
  independently; the user chose FIX. Neither is one of the three purposes above, which is precisely why
  the authorisation is written down instead of inferred from the file already being open.
...
- `CLAUDE.md` — **IN, one number.** Its `withBusyRetry` site count said "six enumerated sites", which
  THIS change made stale by adding the seventh; corrected to seven in review pass 4. Listed because the
  inventory is canonical and a file edited outside it is an unauthorised edit even when the edit is
  right — reverting would have restored a falsehood in the file every session loads. Found by a
  repo-wide sweep for the CLAIM (`adr/066`), not by a guard: `tests/busy-site-count.test.js` reads only
  `adr/020` and `job-busy.mjs`, so this instance was invisible to it through three passes. **Extending
  that guard to `CLAUDE.md` is deliberately NOT done here** — it is the live top-tier item **OAI-110**,
  and doing it here would widen this change into the item that already owns it.
- **OUT: `scripts/lib/job-spawn.mjs`** — decision 4. The root fix is OAI-145.

**STANDING CLAUSE — tracker and provenance files are authorised by this clause, not enumerated.**
`BACKLOG.md`, this plan, and `plans/<id>.approved/` change as a consequence of running the process
rather than as part of the change being reviewed, and an inventory that lists them invites a reviewer
to audit them as code. Three separate passes have now raised the same divergence — first `adr/014`,
then `job-launch-outcome.mjs`, then `BACKLOG.md` — and the first two were genuine omissions of
reviewable artifacts, which is why they were fixed individually. This third class is not, so it is
closed once by rule instead of by a third entry (`adr/066`: fix the claim, not the instance).
