...
- A case that failed every run in a record (no `score`): the matrix cell names the failure
  sub-states present, each with its run count, never a fabricated `0` — the records distinguish
  "found nothing" from "never answered", and so must the reader. A case can fail several ways across
  its runs, so a single winning label would conceal the rest; `failureCells` decomposes the row's
  sub-counts into disjoint buckets — residual `failed` (`failed − timedOut − substituted`), non-cap
  `timeout` (`timedOut − capped`), `capped`, `substituted`, `unreadable`, and `truncated` (an
  answered-but-cut run that still never scored) — and joins the present ones (e.g. `— 2 failed, 1
  substituted`). Metacharacter-free so `safeInline` passes the cell through untouched.
- A control case (`row.control`): no recall — the cell reads `— control` (paren-free, since parens
  are in `safeInline`'s escape class); its `unmatched` is a false-positive count, aggregated into the
  per-record control-FP figure, never into recall. When a control's runs ALL failed (nothing scored),
  the cell names the failures beside the marker (`— control, <failure cells>`), so a control that
  never ran is not read as one that measured clean.
- **A render names the absence of a measurement, never a `0` or a bare marker that reads as one.**
  A per-case cell or an aggregate column whose underlying measurement did not happen reads `—`:
  recall over zero opportunities, throughput with no measurable run, and the Unmatched / Control-FP
  columns when no run scored (`fpText` — the count is 0 by construction there, summed over the scored
  bucket alone, so `—` hides nothing). This mirrors the single-record report's own em-dash
  convention; a new sink of this class is a rule violation, not a fresh design question.
...
1. `npm test` green, and the new `tests/bench-compare.test.js`:
   ...
   - **Failure cells:** an all-failed case renders its failure state, not `0`; a case that failed
     several ways across its runs names every sub-state with its count (a mixed substituted+failed
     case shows `2 failed, 1 substituted`, not just the top-priority label).
