...
only `collectStream` holds (its `firstTextAt`). `bench/lib/sweep-outcome.mjs` needs no edit (its
...
accumulator to whatever the loop throws. `tests/structure.test.js` does not list
`stream-collect.mjs` or `completion.mjs` in `RESPONSE_BOUNDARY_FILES`; the new throw interpolates
...
- it has **no** `choices` property. A frame carrying `choices` is a completion chunk whatever else
  it carries, and stays on the completion path.
...
while `firstTextAt === null` — the same condition the timing and the reserve watchdog already treat
as "generation has not begun". When it holds:
...
- build `new UserError(`${profile.name} refused the request inside the stream.`, { hint: <literal>,
  ...
  an error instead of a completion; its text follows this message; the request was not re-sent
  because a refusal before generation is refused again;
...
Every replay uses the observed bytes verbatim from `evidence/013.md`: the `event: error` line, the
`data:` line with the LM Studio overflow sentence (*The number of tokens to keep from the initial
prompt is greater than the context length…*), a blank line, then `end()` with no `[DONE]`. Because
...
1. `tests/stream-error-frame.test.js` (new), driven through `scriptedServer`-style fake server +
   ...
   - **shape controls** through `errorFrame` directly: `{"choices":[…],"error":{…}}` → null;
     `{"error":null}` → null; `{"error":""}` → null; `{"error":{"message":"x"}}` → `"x"`;
     `{"error":"x"}` → `"x"`.
2. `tests/attempt-response-sites.test.js` — one new `SITES` row: `['stream-collect.mjs — an error
   frame before any text', errorFrameScript, 'stream-error-frame']`, using the same raw bytes.
