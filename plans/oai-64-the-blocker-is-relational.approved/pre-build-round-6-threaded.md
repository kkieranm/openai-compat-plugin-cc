...
condition 4 exists, since a future editor would read it as redundant, and records the one imprecision
knowingly accepted: the witness row is only required to be *behind* the head, not to be capable of
running itself, so a local queued row that is malformed or whose own waiter is dead still admits a
blocker. The adopted marker wording is a necessary-condition claim and stays true of that case; the
alternative — qualifying the witness by its own liveness — would make the display disagree with
`decide`, which blocks such a row too.
...
- **A witness that cannot itself run.** Condition 3 requires the local row to be behind the head, not
  to be viable. A local queued row that is malformed — which survives `reconcileAll` on the *writable*
  path too, so this is not only the read-only case — still admits a blocker. Accepted: the marker
  states a necessary condition, which stays true of such a row, and `decide` blocks it as well, so
  qualifying the witness by its own liveness would make the display disagree with the queue.
