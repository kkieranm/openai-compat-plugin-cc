...
## Docs-step debt (consolidated during the review ladder, apply at the docs step)

The review passes corrected several in-code claims that also appear in, or now require additions to,
CLAUDE.md's multi-pass architecture paragraph. Apply at the docs step:

- **"a thrown pass observed no tokens"** — CLAUDE.md's review-passes paragraph restates this
  (`usage` is a fail-closed sum over READABLE passes (a thrown pass observed no tokens; ...)). The
  Pass-3 thrown-pass usage branch surfaces a thrown pass's usage in `passes[]`, so the phrase is now
  false in code (fixed in `review-passes.mjs` totalDuration comment) and must be corrected in
  CLAUDE.md too — say "a non-observation's tokens (thrown or parse-null) are excluded from the
  top-level sum, surfaced in `passes[]`".
- **"nothing is concealed"** — the `caveatUnion` and `reportPasses` docstrings' phrasing stays
  aspirational for the THROWN subset (an `{ok:false, error, ledger, durationMs}` outcome carries no
  salvaged/degraded to surface; a thrown pass's salvage attempt survives only in `attempts`). F4 made
  it true for the parse-null subset. If the docs restate "nothing is concealed", qualify it — and note
  the model case too: a thrown pass's SERVED MODEL is not surfaced either, so a substituted-and-failed
  pass's substitution is invisible in the envelope (see the filed residue item below).
- **New merged-envelope top-level fields** — CLAUDE.md's paragraph predates the Pass-3 additions
  (`reasoning`, `sampling`, `contextWindow`/`contextSource`/`detectedWindow`/`serverConfig`,
  `degraded`) and the graduation test that enforces additive-equivalence to `jsonReport`. Add them.
- **`--passes` bench wiring residue (PL3-1 / OAI-9 deferred)** — `bench/run.mjs` neither accepts nor
  forwards `--passes`, so `bench/lib/compare-model.mjs`'s `passes` axis is inert today. When the
  wiring lands: the writer must persist `options.passes`, AND the axis should derive multiplicity from
  the report envelope's `kind`/`passCount` (like `capAxis` derives from rows) rather than trusting the
  declared option — four review instances flagged this writer-discipline dependency. **Also, the merged
  SUCCESS envelope must gain a top-level `attempts` aggregate and a truncation signal (`finishReason`,
  or an `atCap`/finish-`length` flag) before bench can read a multi-pass record honestly**:
  `bench/lib/attempt-rows.mjs` reads top-level `report.attempts` (a multi-pass success record would
  read `[]` — every physical request vanishing from the reliability table) and `bench/lib/run-buckets.mjs`
  `truncatedRuns` reads top-level `report.finishReason === 'length'` (a union whose readable passes all
  guillotined would enter `scoredRuns` where its single-pass counterpart is excluded). The
  `PER_PASS_ONLY` allowlist rationale in `tests/review-multi-pass.test.js` must be revisited at the same
  time. Latent today (no multi-pass bench record can exist), surfaced by the OAI-9 terminal review pass.

## Step-8 residue to file (named silent-failure mechanisms, clear the worth bar)

Both are "enrich what a thrown pass's error carries/surfaces" — file together or as siblings:

- **A substituted model on a FAILED pass is concealed.** `servedModelFailure` checks only readable
  passes, and the thrown `passEnvelope` branch does not surface the served model, so a pass served a
  different model than requested that then FAILS (e.g. a stream drop after the model frame — the id is
  on `error.answer.model`) has its substitution invisible in the merged envelope; a `--json`/bench
  identity reader sees only the readable passes' confirmed pair. Findings provenance is NOT concealed
  (the union is all-confirmed-model; a failed pass contributes none) — this is an absent diagnostic on
  a failed pass. Fix candidate: surface `error.answer.model` (server-named, no `modelReported` beside
  it) on the thrown branch, and/or extend `servedModelFailure` to inspect non-observations' carriers.
- **Reason-at-source for reasoning-only (F6).** `requireAnswer`'s reasoning-only/empty-answer refusals
  set no `reason`, so `allFailedError` stamps the coarse `all-passes-unreadable` where a precise
  `reasoning-only`/`empty-answer` would classify better for the sweep's `starved` bucket. Fix
  candidate: set the reason at source in `requireAnswer` (wider blast radius — sweep-outcome vocabulary
  already knows `reasoning-only`, but `empty-answer` may be new), which also fixes `passReason`'s
  generic `unreadable` fallback for these shapes in `passes[]`.
