...
- The bare-empty shape carries `reason: 'empty-answer'` — a NEW code, minted here (review-ladder
  pass 1, `codex-adversarial`, confidence 0.99, replacing round 2's approved `null`): the branch
  POSITIVELY identifies the shape — an answer channel that arrived and held only whitespace — and
  persisting `null` had `bench/lib/attempt-rows.mjs:127` rendering a known cause as
  `unclassified`, the known-cause-reads-as-unknown hole. The code is deliberately NOT
  `empty-completion` or `blank-completion` (`failure-shape.mjs` owns both, with transport-shape
  semantics and membership in `RETRYABLE` and `COMPLETION_SHAPES`, which feeds `serverUnwell` —
  reusing either would misclassify a model-behaviour failure as a server-health symptom);
  `empty-answer` sits in no reason `Set` anywhere, so it is non-retryable and non-starved by
  construction. The server's unvalidated `finish_reason` still travels on `error.finishReason`,
  never in `.message`, per the OAI-185 discipline. **Reachability, verified against disk after round 1's dual dissent
...
- The `attemptSalvage` docstring's "`content`, not `isReasoningOnly`" paragraph stays — it
  justifies the GATE, which is unchanged — and gains a bare pointer to `salvageEmptyFailure`,
  which owns the dispatch and its taxonomy prose in one home (amended after /simplify
  deduplicated the restated table out of the pointer site; review-ladder pass 1 fork finding).
...
3. New test: a salvage follow-up with a clean finish and WHITESPACE-ONLY content (e.g. a
   ...
   empty at the gate, and carries no reasoning) leaves the losing entry `failed` /
   `'empty-answer'` — the bare arm's one reachable route (round 1, both verdicts: a 0-char both-channels fixture is
...
2. A clean-finish, reasoning-bearing, empty-content follow-up still persists `failed` /
   `'reasoning-only'`.
3. A clean-finish, whitespace-only-content follow-up (no reasoning) persists `failed` /
   `'empty-answer'` — the reachable form of the bare arm; a 0-char both-channels reply is out of
   reach by design, refused as `blank-completion` inside `chatCompletion`.
