...
if [ -f "$dir/model" ]; then
  model=$(node -e '
    const fs = require("fs");
    let buf;
    try { buf = fs.readFileSync(process.argv[1]); } catch (e) { process.exit(2); }
    let s;
    try { s = new TextDecoder("utf-8", { fatal: true }).decode(buf); } catch (e) { process.exit(4); }
    if (s.endsWith("\n")) s = s.slice(0, -1);
    if (!s) process.exit(3);
  ...
  case "$?" in
    ...
    4) echo "refusing: model id is not valid UTF-8"; exit 1 ;;
...
on `\xff\xfe`, decodes `qwen3.8-27b` and the U+0085 case from finding 5b unchanged) — refused with
its own exit code and message, distinct from the control-character and empty-file cases. Every one
of these six findings has its own regression test in `tests/delegate-template.test.js` (see Tests
below, amended to match).
...
**Amended alongside the section 2 rewrite above, to cover each of the six findings that drove it,
...
2. Mutation checks, one per closed finding, each following the same back-up/mutate/confirm-fails/
   ...
   `TextDecoder` decode with `buf.toString("utf8")`, confirm only the invalid-UTF-8 test fails. The
   shell-metacharacter test itself — run once, as written, against the real recipe — is the injection
   proof: there is no toggleable guard to disable here (safety comes from the file-based channel
   never touching shell interpolation at all, not from a conditional check on the value), so a
   mutation cycle would have
