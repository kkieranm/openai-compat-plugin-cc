...
| structured-output (effective) | `row.degraded` / `row.reported` | same requested flag but differing degradation ⇒ did NOT run under the same constraint ⇒ suppress |
...
- **Case identity is the whole definition, not the id or just the ground truth (Codex rounds 3-4).**
  ...
  `caseDef`** (stable key order, stable array order) per shared id — enumerating a subset is exactly
...
- **`bench/lib/compare-model.mjs`** (new, pure) — the round-1 simpler-core shape (Codex):
  - `normalizeReviewRecord(entry)` where entry is `{path, stamp, record}` — **validates only the
    minimum top-level shape** (`results` a non-empty array; each element an object with a `caseDef`
    object and a `runs` array) and wraps `caseRows(record.results, {cold})` in try/catch; on any
    failure returns `{incompatible: true, reason}` rather than throwing (a month-old corpus must not
    crash the run — Codex round 1). **It must NOT require each run to carry `score` or `error`** — a
  ...
  - `buildComparison(normalized[], {baseline})` — per-record aggregate (recall = Σfound / Σopportunities
    ...
    **representative generation throughput = median of `row.rate.values` across scored cases**, a
    caseRows field with an explicitly stated statistic — `durationMs` is NOT a caseRows field, so the
    round-0 "median durationMs" is dropped). Computes `rankable` (all axes `known` and equal, equal
    ...
    over non-control) **descending**; ties broken by total `unmatched` ascending (fewer false
    positives), then control-FP total ascending, then `stamp` lexicographic as the final
    deterministic tiebreak. A fully-degenerate record (every case failed ⇒ Σopportunities = 0) guards
    the `0/0` and renders recall as `—`, sorting last (Claude round-2 note). No I/O, no markdown —
    unit-testable.
...
- Two entries with the same resolved stamp (same file passed twice): keep both, disambiguated by the
  ...
  useful end but not an error, and the label stays deterministic.
...
1. `npm test` green, and the new `tests/bench-compare.test.js`:
   ...
   - **Structure test:** `compare-report.mjs` is in `SWEEP_RENDER_FILES` and passes the enforced
     ...
     step 5).
2. **Real run:** `node bench/compare.mjs` against two real `bench/results/*.json` records of
   different models — confirm the ranking renders when they match and is suppressed with a named axis
   when they don't (pick two records with different `cold`/cap). Quote the observed output.
